...
`THESIS_ACCEPTANCE_SPEC.md` §4 lists the required set.

**Partly resolved by this review.** Figures 4.1–4.3 are now provided in `figures/`, generated by
`supervision/make_result_figures.py` directly from `locked_data.json` at 300 dpi in TIFF and PNG,
in greyscale for black-and-white printing, with captions in `figures/CAPTIONS.md`. They are
produced from the locked data rather than re-typed numbers specifically so a figure cannot drift
away from its table — regenerate them rather than editing them by hand.

Two presentation points worth noting, because they are the kind of thing an examiner asks about:

- In Figure 4.1 the Beautifil Flow Plus X F00 standard deviation (3.14) exceeds its mean (1.49),
  so the lower error bar falls below zero. The axis deliberately shows this rather than clipping
  at zero, because clipping would understate the dispersion the outlier causes. The caption states
  that it does not imply a weight gain. This figure is the clearest single explanation of why the
  ANOVA is non-significant.
- In Figure 4.2 the Beautifil II LS post-brushing error bar visibly crosses the 0.2 µm threshold
  line, which is exactly the point Chapter 5 §5.2 makes in words.

**Still required from the candidate:** Figures 3.1 (Teflon mould and disc specimen), 3.2
(toothbrushing simulator), 3.3 (profilometer). These are photographs and cannot be generated.
...
8. Embed Figures 4.1–4.3 from `figures/` into Chapter 4 with the captions from
   `figures/CAPTIONS.md`; obtain Figures 3.1–3.3 from the candidate (B6).
